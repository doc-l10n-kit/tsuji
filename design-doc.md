...
tsuji は、[quarkus.io](https://quarkus.io) ドキュメントサイトの翻訳のために設計されたローカリゼーションツールキットです。gettext PO ファイルを介して、LLM（Google Gemini / OpenAI）または機械翻訳 API（DeepL）による翻訳と、既存の翻訳メモリ（TMX）を活用した RAG（検索拡張生成）を組み合わせ、一貫性の高い翻訳プロセスを提供します。
...
入力は PO ファイル全体（`Po`）で、翻訳済みの `Po` を返します。実装は以下の 3 つです：

| 実装 | 翻訳エンジン | RAG 対応 | マークアップ保護戦略 |
|---|---|---|---|
| `GeminiTranslator` | Google Gemini (LangChain4j) | 対応 | AsciiDoc ネイティブ + 事後バリデーション |
| `OpenAiTranslator` | OpenAI (LangChain4j) | 対応 | AsciiDoc ネイティブ + 事後バリデーション |
...
翻訳時に使用する言語コードは以下の優先順位で解決されます：

1. CLI の `--source` / `--target` 引数（最優先）
2. `tsuji.translator.language.source` / `target`（翻訳 API 固有コード）
3. `tsuji.language.from` / `to`（汎用設定）

DeepL のように API が特定のコード形式（`EN`, `JA` 等）を要求する場合は、`translator.language` で API 互換のコードを指定できます。

### 3.3 パイプラインアーキテクチャ

各 Translator は共通の `MessageProcessor` インターフェースによるパイプラインで翻訳を行いますが、**パイプラインの構成は大きく異なります**。
...
#### LLM ベース Translator のパイプライン（Gemini / OpenAI 共通、2 プロセッサ）

Gemini・OpenAI は AsciiDoc マークアップを直接理解できるため、HTML 変換は不要です。両者は同一のパイプライン構成を持ちます。

```
PoMessage → TranslationMessage 変換
  ↓
1. GeminiTranslationProcessor / OpenAiTranslationProcessor   … バッチ翻訳 + メッセージ分類
2. XrefTitlePostProcessor                                    … <<Title>> → <<section-id,翻訳タイトル>> 変換
  ↓
TranslationMessage → PoMessage 復元
```

`GeminiTranslationProcessor` と `OpenAiTranslationProcessor` は共通基底クラス `AbstractLangchain4jTranslationProcessor` を継承しており、バッチ処理・バリデーション・リトライ・エスカレーションのロジックを共有しています。差分は翻訳 API の呼び出し部分のみです。
...
### 3.4 TranslationMessage ワーキングモデル
...
`AbstractLangchain4jTranslationProcessor` は翻訳実行前にメッセージを 3 種類に分類します：
...
複数のテキストを JSON 配列としてまとめて送信します。`MessageTypeNoteGenerator` がメッセージの種類（見出し等）に応じた翻訳ガイダンスを付与します：
...
```json
{
  "translations": [
    ...
    {"index": 1, "translation": "はじめに"}
  ]
}
```

`TranslationAiService` がレスポンスをパースし、index の一致を検証します。index が不一致の場合は例外を投げます。

#### システムプロンプトの構成

`TranslationAiService` はテンプレート方式でシステムプロンプトを構築します：

```
翻訳システムプロンプト（テンプレート）
  ├── {srcLang} → ソース言語コード
  ├── {dstLang} → ターゲット言語コード
  ├── {glossary} → 用語集（有効時のみ注入）
  └── {additional_rules} → AsciiDoc / HTML マークアップルール（コンテンツに応じて切替）
```

各テンプレートはデフォルトでクラスパス上の組み込みファイルを使用しますが、設定で外部ファイルパスを指定してオーバーライドできます。

#### RAG 対応バッチ翻訳

RAG 有効時は `RAGTranslationAiService` を使用します。各テキストに対してベクトル検索で類似翻訳を取得し、`tm` フィールドとしてリクエストに含めます：
...
`BatchedExecutor` が `BatchProvider` と連携し、バッチ処理とリトライを制御します：

- `peekNext()` で次のバッチを取得します（位置は進めません）
- 成功時: `consumeNext()` で位置を進め、`notifySuccess()` でサイズを増加させます
- バリデーションエラー時: `notifyValidationError()` でサイズを縮小し、同じ位置からリトライします（最大 `max-message-validation-retries` 回）
- 一般エラー時: 同じバッチを指数バックオフでリトライします（最大 `max-retries` 回）
...
#### LLM（Gemini / OpenAI）: 事後バリデーション方式

LLM にはプロンプトで AsciiDoc マークアップの保持を指示しますが、LLM の出力は確定的でないため、`AsciidocMarkupValidator` で翻訳後に検証します。
...
2. jsoup で DOM を解析し、以下のマークアップ特徴量を抽出します：
   - リンクの `href` 集合
   - 画像の `src` 集合
   - `<em>`, `<strong>`, `<code>` の出現回数とテキスト内容
3. 原文と翻訳文の特徴量を比較し、不一致があれば `AsciidocMarkupValidationException` を投げます

**リトライとエスカレーション**: マークアップ破損が検出された場合、破損メッセージのみを再翻訳します（最大 `max-message-validation-retries` 回、デフォルト 4 回）。再翻訳時は以下の情報を LLM に提供します：

- 原文
- 前回の翻訳結果（`previousTranslation`）
- 破損内容の詳細な説明（どのマークアップが不一致か、CJK スペーシングルールへの注意等）

さらに、エスカレーションモデルが設定されている場合、リトライ時にはプライマリモデルからエスカレーションモデルに切り替えます。これにより、初回翻訳はコスト効率の良いモデル（例: Gemini Flash）で行い、リトライ時のみ高品質なモデル（例: Gemini Pro）を使用するという運用が可能です。

それでも修復できない場合は警告コメントを付与して続行します。
...
`po machine-translate` コマンドの `--asciidoc` オプションで AsciiDoc マークアップ処理のモードを指定できます：

| モード | 動作 |
|---|---|
| `auto`（デフォルト） | PO ファイル名が `.adoc.po` で終わる場合に有効化 |
| `always` | 常に AsciiDoc 処理を有効化 |
| `never` | AsciiDoc 処理を無効化 |

`auto` モードにより、`.md.po`、`.html.po`、`.yaml.po` 等の非 AsciiDoc コンテンツに対する不要な AsciiDoc 処理を防ぎます。
...
`RAGTranslationAiService` がテキストごとに `EmbeddingStoreContentRetriever` で類似翻訳を検索します。
...
| `po purge` | fuzzy メッセージの訳文クリア（`--all` で全メッセージ） |
| `po update-stats` | 翻訳進捗の統計出力（メッセージ数・単語数ベース） |
...
LLM ベースの翻訳エンジン（Gemini / OpenAI）は `AbstractLangchain4jTranslationProcessor` を共通基底として、バッチ処理・バリデーション・リトライ・エスカレーションのロジックを共有しています。各翻訳エンジン固有の差分は API 呼び出し部分のみに限定されます。
