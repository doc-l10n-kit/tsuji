...
### モジュール構成
- **tsuji (ルートモジュール)**: CLI アプリケーション本体
- **tsuji-po**: PO (Portable Object) ファイルのドメインモデルと I/O ライブラリ
- **tsuji-tmx**: TMX (Translation Memory eXchange) ファイルのドメインモデルと I/O ライブラリ

### レイヤー構成（tsuji ルートモジュール）
...
3. **Core Driver レイヤー (`net.sharplab.tsuji.core.driver`)**
   - 外部ライブラリ（AsciidoctorJ）や外部API（LangChain4j）、外部プロセス（Git, po4a）とのインターフェースです。
   - PO/TMX ファイル I/O は専用モジュール（tsuji-po, tsuji-tmx）に委譲します。

## 主要コンポーネント

### PoService & PoDriver
- **PoService**: POファイル内のメッセージが翻訳対象かどうかの判定、原文ファイルのパス解決、翻訳進捗の統計計算（メッセージ数/単語数ベース）などのドメイン知識を持ちます。
- **PoDriver**: `tsuji-po` モジュールの `PoCodec` をラップするファサードです。`PoCodec` は `jgettext` ライブラリを使用して物理的なファイルの読み書きを行います。
- **tsuji-po モジュール**: `Po`, `PoMessage`, `PoFlag`, `MessageType` などのドメインモデルと、`PoCodec` (I/O)、`MessageClassifier` (ユーティリティ) を提供します。

### TmxService & TmxDriver
- **TmxService**: POファイル群から TMX (Translation Memory eXchange) モデルを生成する際のフィルタリングや、重複排除のロジックを持ちます。
- **TmxDriver**: `tsuji-tmx` モジュールの `TmxCodec` をラップするファサードです。`TmxCodec` は Jackson XML を使用してTMXファイルのパースと保存を担当します。
- **tsuji-tmx モジュール**: `Tmx`, `TranslationUnit` などのドメインモデルと、`TmxCodec` (I/O)、`TranslationIndex` (検索機能) を提供します。
